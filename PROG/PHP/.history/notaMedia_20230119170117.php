<?php
    ...
    for($may=0;$may<=5;$may++){
        for($men=0;$men<=5;$men++){
            if($nota_alumnos[$may][$mey] < $nota_alumnos[$may][$men+1]){
                $mayorAsignatura = $nota_alumnos[$may][$men];
            };
            if($nota_alumnos[$may][$men] > $nota_alumnos[$may][$men+1]){
                $menorAsignatura = $nota_alumnos[$may][$men];
            };
        };
    };
    echo "<br />";
